...
struct CorkTriangle : public MinimalTriangleData, public BoolTriangleData
{
...
void corkTriMesh2CorkMesh(CorkTriMesh in, CorkMesh *mesh_out)
{
	...
	for (uint i = 0; i < in.n_triangles; i++)
	{
		...
		max_ref_idx = std::max(std::max(max_ref_idx, in.triangles[3 * i + 0]), std::max(in.triangles[3 * i + 1], in.triangles[3 * i + 2]));
	}
