...
template<class T>
MemPool<T>::MemPool(MemPool &src): chunk_list(src.chunk_list), free_list(src.free_list){}
...
MemPool<T>::~MemPool()
{
	// We can ignore the free list here and just go through destroying all of the chunks
...
void MemPool<T>::clear()
{
	// reset the free list
	free_list = NULL;
	for (Chunk *chunk = chunk_list; chunk != NULL; chunk = chunk->next) {
...
void MemPool<T>::addChunk()
{
	...
	// we always double the size of each new chunk.  This ensures that the total # of times that we have to add chunks is logarithmic.
